...
const char* PluginManager::CheckPluginCompatibility(const SKSEPluginVersionData& version) {
	__try {
		...
		if (
			versionIndependent &&
			(RUNTIME_VERSION >= MAKE_EXE_VERSION(1, 6, 629) /*RUNTIME_VERSION_1_6_629*/) &&
...
void PluginManager::ReportPluginErrors() {
	...
	int result = MessageBox(0, message.c_str(), "SKSE Plugin Loader (" __PREPRO_TOKEN_STR__(SKSE_VERSION_INTEGER) "." __PREPRO_TOKEN_STR__(SKSE_VERSION_INTEGER_MINOR) "." __PREPRO_TOKEN_STR__(SKSE_VERSION_INTEGER_BETA) ")",
							MB_YESNO);

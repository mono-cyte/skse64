...
bool DIHookControl::_IsKeyPressed(KeyInfo* info, UInt32 flags) {
	...
	// data sources
	if (flags & kFlag_GameState)
		result |= info->gameState;
	if (flags & kFlag_RawState)
		result |= info->rawState;
	if (flags & kFlag_InsertedState)
		result |= info->insertedState;
...
void DIHookControl::SetKeyDisableState(UInt32 keycode, bool bDisable, UInt32 mask) {
	if (!mask)
		mask = kDisable_All; // default mask value

#include "hooks/Hooks_Scaleform.h"
#include "hooks/Hooks_Gameplay.h"
#include <Utilities.h>
...
#include "scaleform/ScaleformState.h"
#include "internal/GlobalLocks.h"
#include "api/GameMenus.h"
#include "internal/HashUtil.h"
#include "internal/Translation.h"
...
void Hooks_Scaleform_Commit(void) {
	...
	//// fav menu data hooks
	{
		struct SetItemDataEntry_Code : Xbyak::CodeGenerator {
			SetItemDataEntry_Code(void* buf) :
...

	{
		struct SetMagicData_Entry_Code : Xbyak::CodeGenerator {
			SetMagicData_Entry_Code(void* buf) :
				Xbyak::CodeGenerator(4096, buf) {
				// The hooked call (overwritten code)  movieView->CreateObject(containerObj,0,0,0)
				call(ptr[rax + 0x68]);
...

	{
		struct SetVampireData_Entry_Code : Xbyak::CodeGenerator {
			SetVampireData_Entry_Code(void* buf) :
				Xbyak::CodeGenerator(4096, buf) {
				// rdi: FavData* (this)

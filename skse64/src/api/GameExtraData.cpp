...
struct GetMatchingEquipped {
	...
	GetMatchingEquipped(FormMatcher& matcher, bool isWorn = true, bool isWornLeft = true) :
		m_matcher(matcher), m_isWorn(isWorn), m_isWornLeft(isWornLeft) {
		m_found.pForm	   = NULL;
...
public:
	HotkeyDataFinder(SInt32 hotkey) :
		m_matchHotkey(hotkey) {
		m_matchForm = NULL;

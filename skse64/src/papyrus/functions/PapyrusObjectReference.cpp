...
#include "api/GameExtraData.h"
#include "api/GameData.h"
#include "api/GameThreads.h"

#include "internal/InternalTasks.h"

// #include "netimmerse/NiNodes.h"
...
public:
	ExtraContainerFormList::ExtraContainerFormList(BGSListForm* list) :
		m_formList(list) {}
...
public:
	ExtraContainerArray::ExtraContainerArray(VMResultArray<TESForm*>* list) :
		m_array(list) {}
...
public:
	ExtraContainerInfo(EntryDataList* entryList) :
		m_map(), m_vec() {
		m_vec.reserve(128);
		if (entryList) {
...
public:
	ContainerCountIf(ExtraContainerInfo& info) :
		m_info(info) {}
...
public:
	ContainerFindNth(ExtraContainerInfo& info, UInt32 findIndex) :
		m_info(info), m_findIndex(findIndex), m_curIndex(0) {}
...
public:
	ContainerTotalWeight(ExtraContainerInfo& info) :
		m_info(info), m_totalWeight(0.0) {}
...
public:
	ExtraContainerFiller(ExtraContainerInfo& info, ExtraContainerReceiver* receiver) :
		m_info(info), m_receiver(receiver) {}

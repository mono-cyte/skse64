...
namespace {
	class PapyrusModEvent : public ISKSEObject,
	...
	private:
		struct Argument {
			...
			explicit Argument(SInt32 arg) :
				type(kType_Int) { data.i = arg; }
			explicit Argument(float arg) :
				type(kType_Float) { data.f = arg; }
			explicit Argument(bool arg) :
				type(kType_Bool) { data.b = arg; }
			explicit Argument(BSFixedString arg) :
				type(kType_String) { data.str = arg.data; }
			explicit Argument(TESForm* arg) :
				type(kType_Form) { data.form = arg; }

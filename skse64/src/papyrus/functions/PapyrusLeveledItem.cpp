...
#include "api/GameReferences.h"

namespace papyrusLeveledItem {
	UInt32 GetChanceNone(TESLevItem* leveledItem) { return leveledItemUtils::GetChanceNone(&leveledItem->leveledList); }

	void SetChanceNone(TESLevItem* leveledItem, UInt32 chanceNone) { leveledItemUtils::SetChanceNone(&leveledItem->leveledList, chanceNone); }

	TESGlobal* GetChanceGlobal(TESLevItem* leveledItem) { return leveledItemUtils::GetChanceGlobal(&leveledItem->leveledList); }

	void SetChanceGlobal(TESLevItem* leveledItem, TESGlobal* globalVar) { leveledItemUtils::SetChanceGlobal(&leveledItem->leveledList, globalVar); }

	UInt32 GetNumForms(TESLevItem* leveledItem) { return leveledItemUtils::GetNumForms(&leveledItem->leveledList); }

	TESForm* GetNthForm(TESLevItem* leveledItem, UInt32 n) { return leveledItemUtils::GetNthForm(&leveledItem->leveledList, n); }

	UInt32 GetNthLevel(TESLevItem* leveledItem, UInt32 n) { return leveledItemUtils::GetNthLevel(&leveledItem->leveledList, n); }

	void SetNthLevel(TESLevItem* leveledItem, UInt32 n, UInt32 level) { return leveledItemUtils::SetNthLevel(&leveledItem->leveledList, n, level); }

	UInt32 GetNthCount(TESLevItem* leveledItem, UInt32 n) { return leveledItemUtils::GetNthCount(&leveledItem->leveledList, n); }

	void SetNthCount(TESLevItem* leveledItem, UInt32 n, UInt32 count) { return leveledItemUtils::SetNthCount(&leveledItem->leveledList, n, count); }
} // namespace papyrusLeveledItem

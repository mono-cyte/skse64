...
namespace papyrusLeveledActor {
	UInt32 GetNumForms(TESLevCharacter* leveledItem) { return leveledItemUtils::GetNumForms(&leveledItem->leveledList); }

	TESForm* GetNthForm(TESLevCharacter* leveledItem, UInt32 n) { return leveledItemUtils::GetNthForm(&leveledItem->leveledList, n); }

	UInt32 GetNthLevel(TESLevCharacter* leveledItem, UInt32 n) { return leveledItemUtils::GetNthLevel(&leveledItem->leveledList, n); }

	void SetNthLevel(TESLevCharacter* leveledItem, UInt32 n, UInt32 level) { return leveledItemUtils::SetNthLevel(&leveledItem->leveledList, n, level); }

	UInt32 GetNthCount(TESLevCharacter* leveledItem, UInt32 n) { return leveledItemUtils::GetNthCount(&leveledItem->leveledList, n); }

	void SetNthCount(TESLevCharacter* leveledItem, UInt32 n, UInt32 count) { return leveledItemUtils::SetNthCount(&leveledItem->leveledList, n, count); }
} // namespace papyrusLeveledActor

...
#include "scaleform/ScaleformCallbacks.h"
#include "scaleform/ScaleformMovie.h"
#include "scaleform/ScaleformAPI.h"
#include "scaleform/ScaleformExtendedData.h"
#include "api/GameMenus.h"
#include "api/GameEvents.h"
#include "api/GameForms.h"
#include "api/GameInput.h"
#include "internal/CustomMenu.h"
#include <float.h>

#include "internal/Serialization.h"

template <>
void SetGFxValue<bool>(GFxValue* val, bool arg) { val->SetBool(arg); }
template <>
void SetGFxValue<float>(GFxValue* val, float arg) { val->SetNumber(arg); }
template <>
void SetGFxValue<UInt32>(GFxValue* val, UInt32 arg) { val->SetNumber(arg); }
template <>
void SetGFxValue<SInt32>(GFxValue* val, SInt32 arg) { val->SetNumber(arg); }
template <>
void SetGFxValue<BSFixedString>(GFxValue* val, BSFixedString arg) {
	// lifetime of this string will not be managed by the scaleform runtime
	val->SetString(arg.data);
}

template <>
bool GetGFxValue<bool>(GFxValue* val) { return (val->GetType() == GFxValue::kType_Bool ? val->GetBool() : false); }
template <>
float GetGFxValue<float>(GFxValue* val) { return (val->GetType() == GFxValue::kType_Number ? val->GetNumber() : 0); }
template <>
UInt32 GetGFxValue<UInt32>(GFxValue* val) { return (val->GetType() == GFxValue::kType_Number ? (UInt32)val->GetNumber() : 0); }
template <>
SInt32 GetGFxValue<SInt32>(GFxValue* val) { return (val->GetType() == GFxValue::kType_Number ? (SInt32)val->GetNumber() : 0); }
template <>
BSFixedString GetGFxValue<BSFixedString>(GFxValue* val) {

...
namespace Serialization {
	struct PluginCallbacks {
		PluginCallbacks() :
			revert(NULL), save(NULL), load(NULL), formDelete(NULL), uid(0), hadData(false), hadUID(false) {}
	...
	template <>
	bool WriteData<BSFixedString>(SKSESerializationInterface* intfc, const BSFixedString* data);
	template <>
	bool ReadData<BSFixedString>(SKSESerializationInterface* intfc, BSFixedString* data);

	template <>
	bool WriteData<std::string>(SKSESerializationInterface* intfc, const std::string* data);
	template <>
	bool ReadData<std::string>(SKSESerializationInterface* intfc, std::string* data);

	// Note: Read would have to allocate somehow. You have to do that manually.
	template <>
	bool WriteData<const char>(SKSESerializationInterface* intfc, const char* data);

...
std::string GetRuntimePath() {
	...
	// GetModuleHandle(NULL)获取当前进程句柄，路径写入缓冲区
...
/**
 * @brief 获取当前进程的可执行文件名称(含后缀)
 *
 * @return std::string
 */
std::string GetRuntimeName() {
	std::string appPathStr = GetRuntimePath();

	fs::path appPath(appPathStr);

	fs::path appName = appPath.filename();

	return appName.string();
}

/**
 * @brief 获取当前进程的可执行文件所在路径
 *
...
const std::string& GetRuntimeDirectory() {
	...
	if (s_runtimeDirectory.empty()) {
		fs::path runtimePath(GetRuntimePath());


		if (!runtimePath.is_absolute()) {
			_WARNING("no slash in runtime path? (%s)", runtimePath.string().c_str());
		}

		s_runtimeDirectory = runtimePath.parent_path().string();
...
const std::string& GetConfigPath() {
	...
	if (s_configPath.empty()) {
		...
		if (!runtimePath.empty()) {
			...
			fs::path p(runtimePath);
			p = p / "Data" / "SKSE" / "skse.ini";

			s_configPath = p.string();
